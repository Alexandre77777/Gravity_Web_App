...
Интерфейс и вычислительная часть разделены: Streamlit-клиент и FastAPI-сервис запускаются независимо и обмениваются данными по HTTP. К одному API могут обращаться разные программы — например, веб-интерфейс, мобильное приложение или программа на компьютере. При смене размещения сервиса достаточно указать новый базовый адрес API в боковой панели.
